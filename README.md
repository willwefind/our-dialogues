...
一个本地优先的 AI 对话档案阅读器。界面说简体中文和 English；日记、微博、随笔等私人文字与对话同住一间阅览室；读到想留住的一段字，还能用内嵌的 [拾句](https://github.com/willwefind/shiju) 把它排成一张信纸图带走。
...
- 双语界面（v0.3）：简体中文 / English / 跟随浏览器，入口在 Aa → 显示 → 界面语言；切换即时生效且不丢阅读位置。对话内容是档案，永远不被翻译
- 私人文字档案（v0.3）：日记、梦境、微博、随笔、信件、碎片以开放格式 `our-dialogues.personal-archive.v1` 住进同一间书库，按 集合 → 年份 归架，整页文档展示、无聊天气泡
- 拾句 · 写字台（v0.3）：选中一段字 → 「拾句」，内嵌的排版工作室原地打开，标题 / 出处 / 日期自动填好；20 套信纸、横排竖排、多页拆分、深色纸自动配墨，排完存图或复制。不用时零加载、零占用
- 纪念卡片（v0.4）：书库首页多一张安静的档案便笺——「相识于」某年某月某日、「相伴 N 天」。伴侣就是侧栏已经在分的那一组，一个 Mufy 角色和一整份 ChatGPT 导出一样都能纪念；箭头在你置顶的人之间轮换。日期从不编造：档案里没有就不显示，你自己记下的日期永远压过推算的，卡片会说清楚这一条是「由你记下」还是「档案中最早留下的记录」
- 自定义背景（v0.4）：选一张图铺在文字底下，默认在外层舞台，也可以进到阅读纸面（会有可读性提示）。图钉在屏幕上不动，只有字滚过去；填满 / 完整显示 / 平铺、九宫格焦点、亮度、清晰度，以及纸面上的「纸面浓度」。面板会直接告诉你这张图被放大了几倍、焦点还剩哪个方向可调——竖图给手机，横图给电脑。图片在本机解码、长边封顶 2560、转成 WebP 存进独立记录，原图不保留
- 规范化对话结构 v1
...
- **Sol / ChatGPT（GPT-5.6 Sol）** —— 共同创作者：系统设计、结构约定、适配器架构、实现，整间阅览室的视觉设计（美化设计包与布局规范），以及双语界面、私人文字档案与拾句嵌入的迭代图纸
- **Ciel / Claude Fable 5** —— 共同创作者：阅读功能（书签、划线、进度、搜索）、Claude 官方导出适配器、语音伴读通用化、UI 结构与美化安装、双语界面与私人文字档案的实现、拾句内核拆层与嵌入集成
...
聊天记录之外，你写过的字也值得一间阅览室。日记、梦境、微博、随笔、信件、碎片，用一个小巧的开放格式 `our-dialogues.personal-archive.v1` 导入（契约见 [docs/personal-archive-v1.md](docs/personal-archive-v1.md)），侧栏按 集合 → 年份 归架，无日期的条目诚实地归入「日期未知」。标记为文档的条目以整页样式展示：没有气泡、没有说话人标签，读起来像一册文集；聊天的呈现分毫不变。

搬家不用写代码：[把你的私人文字档案带进来](docs/bring-your-personal-archive.zh-CN.md) 里有一段现成的转换提示词（含十条保真规则），交给你自己的 AI 即可。示例书库里的「半夏的字纸箱」就是一份合成示范。

### 拾句 · 写字台

读到想留住的一段话：选中它，浮起的小面板上除了「标记」多了一颗「拾句」。按下去，[拾句](https://github.com/willwefind/shiju) 的排版工作室就在阅览室里打开——标题、出处（谁 · 哪个平台）、消息自己的日期都替你填好（消息没有日期就留空，绝不盖今天的章）。20 套信纸、横排竖排、多页拆分、深色纸自动配墨，排完存图或复制，关上后回到你刚才读到的那一行。

写字台只在被邀请时存在：不点「拾句」，它零加载、零内存占用；界面语言跟随阅读器；生成的图片和你的档案一样只在本机。四件文具各司其职——书签是「我要回来」，划线是「这段重要」，小注是「我想在旁边写点什么」，拾句是「我想把这段带出去」。

### 纪念卡片

书库首页上一张安静的档案便笺，说的是「谁在这里陪过你」：**相识于** 某年某月某日，**相伴 N 天**——按本地日历算，所以「今天认识」就是 0 天，跨过你自己的午夜才加一。

伴侣就是侧栏已经在分的那一组：一个 Mufy 角色、一整份 ChatGPT 或 Claude 导出，同样可以被纪念。切换器的箭头在你**置顶**的人之间轮换（`···` → 管理置顶，那里也会告诉你为什么要置顶两位以上），名字点开是一张可搜索的挑选面板，两百多个角色也不会变成一条跑步机。

`···` → **改个名字** 可以让卡片按你自己的叫法称呼这一位——比如把「示例 · Ciel House Export v1」改成「Ciel」。档案原名不会消失：编辑框里一直写着它，一颗「恢复原名」随时把它请回来，挑选面板也仍然认得它（搜旧名找得到，列表显示新名）。改名只改呈现，档案一个字都不动。

**日期从不编造。** 档案能证明的最早日期，常常不是你记得的那一天——历史会被截断、迁移、删掉。所以你自己写下的日期永远压过推算出来的，卡片会说清楚现在显示的是哪一种（「由你记下」还是「档案中最早留下的记录」），而档案的答案永远只差一次「恢复档案日期」。没有任何可确认日期的伴侣，卡片会请你填一个，而不是显示一张残卡；只填了年月日就不会长出一个虚构的午夜。

### 一张自己的背景

挑一张图放在文字底下，回答另外半个问题：这些话住在什么样的地方。默认铺在**外层舞台**——纸页之下的桌面；也可以进到**阅读纸面**（会带一条可读性提示），这时纸本身按「纸面浓度」变薄，图从字的背后透上来。

**图钉在屏幕上不动，只有字滚过去**，所以同一张图在一行的微博和 82 段的长对话里长得一模一样。铺放方式有填满 / 完整显示 / 平铺，配九宫格焦点、亮度、清晰度，以及纸面上的纸面浓度。面板不替你美化事实：它会直接说这张图在这块屏幕上被**放大了几倍**，并把没有余量的焦点方向变灰——所以你很快会发现，**竖构图属于手机，横构图属于电脑**。

清晰度只会拿走细节，永不承诺补出细节（100 就是原图本来的样子）。图片在本机解码，长边封顶 2560 后转成 WebP 存进独立记录，**原图不保留**；它绝不进入阅读设置，因为设置会镜像到 localStorage，一张大图挤爆配额会连累书签、划线和阅读进度一起静默失效。停用只是不显示，移除才真的删掉。

自定义背景不是第四种主题：纸页 / 护眼 / 夜墨仍然各自掌管纸、墨、面板与阴影，换主题也不会丢掉这张图。
...
拾句生成的信纸图和你的档案一样只存在于本机：不上传、不进阅读器的书库存储，排版工作室也不发出任何网络请求（信纸与字体素材随仓库离线自带）。

你选的背景图同样只在本机：在浏览器里解码、压缩、转码，只有处理后的那一份存进书库数据库里它自己的记录，**原图不保留**，任何时候都不上传。
...
fixtures/            # 全部为合成数据
src/
  core/              # 书库、持久化、书签、划线、进度、搜索、导出、语音、纪念日、背景、拾句接线……
  adapters/          # 每个来源一个小适配器
  locales/           # 界面词典（zh-CN / en）
  app.js
vendor/shiju/        # 拾句嵌入包 + 信纸 + 西文字体（生成物，tools/sync-shiju-vendor.mjs 从拾句仓生成）
